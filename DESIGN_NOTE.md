...
| **`Attempt`** | Represents one distinct learner practice session on a problem. | Belongs to a problem. Preserves chronological history. Unlimited attempts allowed per problem. Starts in `IN_PROGRESS`. |
| **`Submission`** | Represents the actual design artifact submitted by the learner. | Strictly 1:1 with `Attempt`. Immutable upon submission. Holds content type (`TEXT`, extensible to `DIAGRAM`, `CODE`). Saved before evaluation begins. |
...
| **`Feedback`** | Contains structured assessment for a specific rubric dimension. | Cascades from `Evaluation`. Captures `evidence`, `concern`, `suggestion`, `score`, and `maxScore`. Atomically persisted. |
...
```text
  ...
  │    └─────┬─────┘         │
  │          │               │
  │   success│   failure     │ retryEvaluation()
  │          ▼               ▼
  │    ┌───────────┐   ┌───────────┐
  │    │ COMPLETED │   │  FAILED   │
  │    └───────────┘   └─────┬─────┘
...
All other transitions (e.g. `PENDING` → `COMPLETED`, `COMPLETED` → `EVALUATING`, `FAILED` → `COMPLETED`) are rejected with `400 Bad Request`.
...
2. **Evaluation Initialized**: An `Evaluation` record is created in `PENDING` or `EVALUATING` status.
3. **Evaluator Execution**: The LLM is called with structured JSON output schema and grounded prompts.
4. **On Success**:
   - Total score is calculated deterministically from criterion scores.
   - `Feedback` records are saved in an atomic transaction.
...
- **Learner Retry ("Try Again")**: Creates a completely **NEW** Attempt record. The previous attempt, submission, evaluation, and feedback remain 100% intact for comparative analysis.
- **Evaluation Retry**: Used when an evaluation fails due to network/LLM timeouts. It reuses the **SAME** Evaluation record, transitioning it back to `EVALUATING` and re-running the evaluator without duplicating attempts or submissions.
